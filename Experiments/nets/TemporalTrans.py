...
# logger = logging.getLogger(__name__)
#
# # class Temporal_Embeddings(nn.Module):  # todo
# #     """Construct the embeddings from patch, position embeddings.
# #     """
# #     def __init__(self,patchsize, img_size, batch_size):
# #         super().__init__()
...
# #         self.dropout = Dropout(0.5)
# #         coords = torch.stack(torch.meshgrid([torch.arange(batch_size), torch.arange(batch_size)]))
# #         self.relative_coords = coords[0] - coords[1] + batch_size - 1
# #         self.relative_coords = self.relative_coords.view(batch_size, -1)  # [B, B]  8*8
# #         self.relative_coords = torch.register_buffer('relative_coords', self.relative_coords)  # register buffer to save it in the model, trainable=False
# #         self.relative_position_embeddings = nn.Parameter(torch.zeros(2*batch_size-1, n_patches, batch_size))  # trainable parameter
#
#
#     # def forward(self, x):
#     #     if x is None:  # x -> (C, B, H, W)
...
#     #     return embeddings
#
# # class ralative_position(nn.Module):  # todo
# #     def __init__(self, patchsize, img_size, batch_size):
# #         super().__init__()
# #         img_size = _pair(img_size)  # _pair is a function that returns a tuple of 2 elements
# #         patch_size = _pair(patchsize)
...
# #         return embeddings
#
# class Position_Embeddings(nn.Module):  # original
#     """Construct the embeddings from patch, position embeddings.
#     """
...
#                                        kernel_size=patch_size,
#                                        stride=patch_size)
#         self.position_embeddings = nn.Parameter(torch.zeros(1, n_patches, n_channels))  # trainable parameter
#         self.dropout = Dropout(0.3)
#
#     def forward(self, x):
#         if x is None:  # x -> (B, C, H, W)
#             return None
#         x = self.patch_embeddings(x)  # (B, C, n_patches^(1/2), n_patches^(1/2)) hidden = in_channels
...
#         self.softmax = Softmax(dim=3)
#         self.out = nn.Linear(batch_size, batch_size, bias=False)
#         self.attn_dropout = Dropout(0.3)
#         self.proj_dropout = Dropout(0.3)
#
#         coords = torch.stack(torch.meshgrid([torch.arange(batch_size), torch.arange(batch_size)]))
#         relative_coords = coords[0] - coords[1] + batch_size - 1
#         relative_coords = relative_coords.view(-1, batch_size)  # [B, B]  8*8
...
#         if emb is not None:
#             for query in self.query:
#                 Q = query(emb)  # (C, 196, B) -> (C, 196, B), Q
#                 multi_head_Q_list.append(Q)
#         for key in self.key:
#             K = key(emb)  # (C, 196, B) -> (C, 196, B), K
#             multi_head_K_list.append(K)
#         for value in self.value:
#             V = value(emb)  # (C, 196, B) -> (C, 196, B), V
#             multi_head_V_list.append(V)
#         # print(len(multi_head_Q4_list))
#
#         multi_head_Q = torch.stack(multi_head_Q_list, dim=1) if emb is not None else None  # stack of 4 heads alone a new dimension, (C, 4, 196, B)
#         multi_head_K = torch.stack(multi_head_K_list, dim=1)  # (C, 4, 196, B)
#         multi_head_V = torch.stack(multi_head_V_list, dim=1)  # (C, 4, 196, B)
#         # print(f'multi_head_Q: {multi_head_Q.shape}')
#
#         multi_head_Q = multi_head_Q.permute(2, 1, 3, 0) if emb is not None else None  # (C, 4, 196, B) -> (196, 4, B, C)
#         multi_head_K = multi_head_K.permute(2, 1, 0, 3)  # (C, 4, 196, B) -> (196, 4, C, B)
#         multi_head_V = multi_head_V.permute(2, 1, 3, 0)  # (C, 4, 196, B) -> (196, 4, B, C)
#
#         # multi_head_Q = multi_head_Q.transpose(-1, -2) if emb is not None else None  # (C, 4, 196, B) -> (C, 4, B, 196)
#         # multi_head_V = multi_head_V.transpose(-1, -2)  # (C, 4, 196, B) -> (C, 4, B, 196)
#
#         attention_scores = torch.matmul(multi_head_Q, multi_head_K) if emb is not None else None  # (196, 4, B, C) * (196, 4, C, B) -> (196, 4, B, B)
...
#         # print(f'rela_pos: {relative_temporal_bias.shape}')
#
#         attention_scores = attention_scores + relative_temporal_bias  # (196, 4, B, B)  # todo
#
#         # attention_scores = attention_scores.view(-1, self.num_attention_heads, self.KV_size, self.KV_size)  # (196, 4, B, B)
#
#         attention_probs = self.softmax(self.psi(attention_scores)) if emb is not None else None  # attention_probs = softmax(attention_scores/ sqrt(d_k))
...
#         context_layer = torch.matmul(attention_probs, multi_head_V) if emb is not None else None  # (196, 4, B, B) * (196, 4, B, C) -> (196, 4, B, C)
#
#         context_layer = context_layer.permute(3, 0, 2, 1).contiguous() if emb is not None else None  # (196, 4, B, C) -> (C, 196, B, 4), deep copy
#         context_layer = context_layer.mean(dim=3) if emb is not None else None  # (196, C, B, 4) -> (196, C, B) mean along the head dim
#
#         O1 = self.out(context_layer) if emb is not None else None  # out is a linear layer, output dim = input dim
#         O1 = self.proj_dropout(O1) if emb is not None else None
#
#         return O1 # (196, C, B)
#
...
#         org = emb  # (C, 196, B)
#
#         cx = self.attn_norm(emb) if emb is not None else None  # (C, 196, B)
#         cx = self.temporal_attn(cx)  # (C, 196, B) -> (C, 196, B)
#         cx = org + cx if emb is not None else None  # (C, 196, B)
#
#         org = cx
#         x = self.ffn_norm(cx) if emb is not None else None  # layer norm
#
#         x = self.ffn(x) if emb is not None else None  # (C, 196, B)
#
#         x = x + org if emb is not None else None  # residual connection
#
#         return x # (C, 196, B)
...
#         attn_weights = []
#         for layer_block in self.layer:
#             emb = layer_block(emb)  # (C, 196, B) -> (C, 196, B)
...
#     def __init__(self, img_size, n_channels, patchSize=16):
#         super().__init__()
#         self.embeddings = Position_Embeddings(patchSize, img_size=img_size, in_channels=n_channels)
#         self.encoder = Encoder(8)
#         self.reconstruct = Reconstruct(n_channels, n_channels, kernel_size=1,scale_factor=(patchSize, patchSize))
#
#     def forward(self,en):
...
#         # en_p = en.permute(1, 0, 2, 3) # (B, C, H, W) -> (C, B, H, W)
#         # print(en.shape)
#         emb = self.embeddings(en)  # (B, C, H, W) -> (B, 196, C)
#         emb_p = emb.permute(2, 1, 0) # (B, 196, C) -> (C, 196, B)
#         # print(emb.shape)
#
#
#         encoded = self.encoder(emb_p)  # (C, 196, B) -> (C, 196, B)
#         # print(encoded.shape)
#         encoded = encoded.permute(2, 0, 1)  # (C, 196, B) -> (B, C, 196)
#         # print(encoded.shape)
#         x = self.reconstruct(encoded) if en is not None else None  # (B, C, 196) -> (B, C, 224, 224)
#
#
