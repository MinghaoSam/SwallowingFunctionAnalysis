...
def main_loop(batch_size=config.batch_size, model_type='', tensorboard=True):
    ...
    elif model_type == 'CTTUNet':
        config_vit = config.get_CTranS_config()
        model = CTTUNet(config=config_vit, n_channels=config.n_channels, n_classes=config.n_labels)
    ...
    elif model_type == 'DSCNet' or model_type == 'DSCNet_TC':
    ...
    elif model_type == 'DSC_CTransNet':
        config_dsctrans = config.get_CTranS_config()
        model = DSC_CTransNet(config=config_dsctrans, n_channels=config.n_channels, n_classes=config.n_labels)
    ...
    if model_type == 'SwinUnet':            
        ...
        optimizer = torch.optim.Adam(filter(lambda p: p.requires_grad, model.parameters()), lr=lr, weight_decay=0.0001)  # Choose optimize
